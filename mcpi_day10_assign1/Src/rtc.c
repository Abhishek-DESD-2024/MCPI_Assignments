...
{
	//initialize RTC
	RCC->APB1ENR |= RCC_APB1ENR_PWREN; // ENABLE PREIFHERAL CLOCK
	PWR->CR |= PWR_CR_DBP;			   // ENABLE ACCESS TO RTC REGISTER

	RCC->CSR |= RCC_CSR_LSION;			// ENABLE LSI(LOW SPEED INTERNAL OSCILLATOR)
	while(!(RCC->CSR & RCC_CSR_LSIRDY));// WAIT FOR LSI READY FLAG

	RCC->BDCR |= RCC_BDCR_RTCEN | RCC_BDCR_RTCSEL_1 ; // SELECT LSI AND ENABLE RTC
